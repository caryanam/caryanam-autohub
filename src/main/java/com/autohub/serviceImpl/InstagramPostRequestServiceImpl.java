...
import com.autohub.dto.InstagramPostRequestItemResultDTO;
...
@Slf4j
public class InstagramPostRequestServiceImpl implements InstagramPostRequestService {
    ...
    @Override
    @Transactional
    public InstagramPostRequestBulkResponseDTO requestBulkInstagramPost(
            Long dealerId,
            InstagramPostRequestBulkRequestDTO request) {

        List<InstagramPostRequestItemResultDTO> results = new ArrayList<>();
        int accepted = 0;

        for (Long vehicleId : request.getVehicleIds()) {

            InstagramPostRequestItemResultDTO result = evaluateAndCreateRequest(dealerId, vehicleId);
            results.add(result);
        ...
        log.info("Dealer [{}] submitted {} Instagram post requests, {} accepted, {} skipped",
                dealerId, request.getVehicleIds().size(), accepted, request.getVehicleIds().size() - accepted);

        return InstagramPostRequestBulkResponseDTO.builder()
    ...
    private InstagramPostRequestItemResultDTO evaluateAndCreateRequest(Long dealerId, Long vehicleId) {
        ...
        List<VehicleInstagramPostRequest> active = instagramPostRequestRepository.findActiveByVehicleId(vehicleId);

        boolean alreadyPublished = active.stream()
                .anyMatch(r -> r.getPublishStatus() == SocialPostPublishStatus.PUBLISHED);
        if (alreadyPublished) {
            return skipped(vehicleId, "Vehicle already published to Instagram");
        }
        ...
        VehicleInstagramPostRequest saved = instagramPostRequestRepository.save(
                VehicleInstagramPostRequest.builder()
                        .vehicle(vehicle)
        ...
        return InstagramPostRequestItemResultDTO.builder()
                .vehicleId(vehicleId)
    ...
    private InstagramPostRequestItemResultDTO skipped(Long vehicleId, String reason) {
        return InstagramPostRequestItemResultDTO.builder()
                .vehicleId(vehicleId)
                .accepted(false)
                .reason(reason)
    ...
    @Override
    public List<InstagramDealerVehicleStatusDTO> getDealerVehicleStatuses(Long dealerId) {
    ...
    private InstagramDealerVehicleStatusDTO toDealerVehicleStatusDTO(Vehicle vehicle) {

        VehicleInstagramPostRequest latest = instagramPostRequestRepository
        ...
        return InstagramDealerVehicleStatusDTO.builder()
                ...
                .instagramPostUrl(latest == null ? null : latest.getInstagramPostUrl())

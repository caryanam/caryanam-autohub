...
/**
 * One row per "dealer requests this vehicle be posted to the Instagram Page".
 ...
 * is enforced in the service layer - see InstagramPostRequestService.
 */
@Entity
@Table(name = "vehicle_instagram_post_requests", indexes = {
        @Index(name = "idx_vipr_vehicle_id", columnList = "vehicle_id"),
        @Index(name = "idx_vipr_dealer_id", columnList = "dealer_id"),
        @Index(name = "idx_vipr_approval_status", columnList = "approval_status"),
        @Index(name = "idx_vipr_publish_status", columnList = "publish_status")
})
@Getter
@Setter
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class VehicleInstagramPostRequest {
    ...
    @JoinColumn(name = "vehicle_id", nullable = false, foreignKey = @ForeignKey(name = "fk_vipr_vehicle"))
    private Vehicle vehicle;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "dealer_id", nullable = false, foreignKey = @ForeignKey(name = "fk_vipr_dealer"))
    ...
    @Column(name = "instagram_post_url", length = 500)

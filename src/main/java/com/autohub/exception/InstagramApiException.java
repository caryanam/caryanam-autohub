...
/**
 * Thrown when the Instagram Graph API returns a non-2xx response after all
...
public class InstagramApiException extends RuntimeException {
     ...
     * InstagramPostBatchItem.retryable flag. False for transient failures
     * (network, timeout, 5xx, rate limit) that are safe to retry.
     */
    private final boolean permanent;

    public InstagramApiException(String message, String responseBody, boolean permanent, Throwable cause) {
        super(message, cause);
        this.responseBody = responseBody;
        this.permanent = permanent;
    }

    public InstagramApiException(String message, String responseBody, boolean permanent) {
        super(message);
        this.responseBody = responseBody;
        this.permanent = permanent;
    }
